...
const Search = (props) => {
    ...
    const loadData = async (postData) => {
        ...
        const response = await dispatch(_fetchData(HOSTNAME, 'api/quantityunit/search', postData));
        if (!response.iserror) {
    ...
    const onSubmit = (MLObject) => {
        let postData = {
            $or: [
                {
                    quantityunitid: MLObject.keyword
                },
                {
                    quantityunitname: MLObject.keyword
                }
            ]
        }
        loadData(postData);
        ...
        <>
            <SearchForm listColumn={SearchElement} layout='vertical' onSubmit={onSubmit} />
            {
                ...
                <DataGird
                    ...
                    isShowModalBtn={true}
                    TitleModal="đơn vị tính'"
                    listColumnModal={ElementList}
                    onSubmitModel={(values, action) => onSubmit({})}
                    hostName={HOSTNAME}
                    apiAdd='api/quantityunit/add'
                    apiUpdate='api/quantityunit/update'
                    apiDelete='api/quantityunit/delete'
